	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		 ...
		 if(pathInfo == null) { // 多筆查詢
			 ...
			 try {
				 ...
				 Ticket ticket = service.geTicket(id);
				 // 物件轉 json
				 ...
				 // 回應一個 json 格式的錯誤資訊
				 ApiResponse<Ticket> ApiResponse = new ApiResponse<>(false, null, "未輸入 id 值");
				 resp.getWriter().println(gson.toJson(ApiResponse));
			 } catch (Exception e) {
				 System.out.println(e.getMessage());
				// 回應一個 json 格式的錯誤資訊
				 ApiResponse<Ticket> ApiResponse = new ApiResponse<>(false, null, e.getMessage());
				 resp.getWriter().println(gson.toJson(ApiResponse));				 
			 }
